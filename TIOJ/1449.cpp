...
signed main()
{
    ...
    for (int j = 1; j <= K; j++)
    {
        ...
        for (int i = 1; i <= N; i++)
        {
            ...
            dp[i][j] = f(j, dq.front().i, i);

            while (!dq.empty() && binary_search(j, dq.back().i, i, i, N + 1) <= dq.back().L)
                dq.pop_back();

            if (dq.empty())
                dq.push_back(transfer(i, i, N));
            else
            {
                int left = binary_search(j, dq.back().i, i, i, N + 1);
                if (left <= N)
                {
                    dq.back().R = left - 1;
                    dq.push_back(transfer(i, left, N));
                }
            }
        }
    }
