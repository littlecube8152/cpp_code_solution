...
double seg[400005][2][2], p[100005], v[100005], x[100005], ans;
        ...
        for (int r = 0; r <= 1; r++)
        {
            ...
            for (int ml = 0; ml <= 1; ml++)
                for (int mr = 0; mr <= 1; mr++)
                    if (!connect[i][ml][mr])
                        seg[i][l][r] += seg[L][l][ml] * seg[R][mr][r];
        }
}
...
{
    cout << i  << "[" << L << ", " << R << "] " << "|" << seg[i][0][0] << " " << seg[i][0][1] << " " << seg[i][1][0] << " " << seg[i][1][1] << '\n';
    if (L != R)
    {
        int mid = (L + R) / 2;
        debug(i * 2, L, mid);
...
signed main()
{
    ...
    pll cur;
    int qdx = 0;
    res.emplace_back(make_pair(0, 1.0));
    init();
    debug();
    cout << '\n';
    while (qdx < query.size())
    {
        ...
        {
            cout << (query[qdx].S.F >> 1) << " " << (query[qdx].S.S >> 1) << '\n';
