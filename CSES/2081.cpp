...
ll N, L, R, pre[200005], Size[200005], vis[200005], dis[200005], discnt[200005];
vector<int> E[200005];
vector<ll> add;
...
ll solve(int root)
{
    ...
    ll ans = 0;
    vis[centroid] = 1;
    for (int i = 0; i <= Size[centroid]; i++)
        discnt[i] = 0;
    discnt[0] = 1;
    sort(E[centroid].begin(), E[centroid].end(), [&](int i1, int i2)
    ...
    for (int i : E[centroid])
        if (!vis[i])
        {
            ...
            for (int j = 0; j < add.size(); j++)
                if (j <= R)
                {
                    ans += add[j] * (discnt[max(0LL, L - j)] - discnt[R - j + 1]);
                    //cout << "ans " << ans << '\n';
                }
            for (int j = add.size() - 1; j >= 1; j--)
                add[j - 1] += add[j];
            for (int j = 0; j < add.size(); j++)
                discnt[j] += add[j];
        }
